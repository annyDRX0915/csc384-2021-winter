...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        Constraint.__init__(self, name, [qi, qj])
        self._name = "Queen_" + name
        self.satAssignments = []
        for ci in qi.curDomain():
            for cj in qj.curDomain():
                if ci != cj and ci != cj + (i - j) and ci != cj - (i - j):
                    self.satAssignments.append([ci, cj])
    ...
    def hasSupport(self, var, val):
        ...
        for v in self.scope():
            if v.isAssigned():
                ...
                if any(x in self._required for x in v.curDomain()):
                    numSatisfy += 1
        return (assignments <= self._ub) and (self._lb <= numSatisfy)
